...
def get_country():
    ...
    var = tk.StringVar()
    label = tk.Message( r, textvariable=var ,width=200,bg="#464646",fg="#9199a1",pady=0)
    
    var.set("COVID-19 Graphical Representation\ntotal cases in "+country_name+" : "+str(cases)+"\ntotal deaths : "+str(deaths)+"\ntotal recovered : "+str(recovered))
    label.pack()
...
frame = ttk.Frame(r)
frame.pack()
bottomframe = ttk.Frame(r)
bottomframe.pack(side = "left")
e1 = ttk.Entry(frame)
e1.grid(row=0, column=0)
search = ttk.Button(frame, text='search', width=10, command=get_country)
search.grid(row=0, column=2)
btn2 = ttk.Button(r, text ="Button 2", command = rightframe.destroy) 
btn2.pack(pady = 10)
